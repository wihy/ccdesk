...
- Produces:
  ...
  - `to_outcome_record(event: dict) -> dict | None`（`PostToolUse` / `PermissionDenied` → 带 `match_key` 的结局记录）
  - `match_key(session_id: str, tool_name: str, input_fp: str) -> str`
...
class Collector:
    ...
    def run_once(self) -> dict:
        ...
        for rid, rec in self.ledger.read_merged().items():
        ...
        with open(self.events_path, "rb") as fh:
            ...
            for raw in fh:
                ...
                if record is not None:
                    ...
                    by_match[record["match_key"]] = record["req_id"]
                    stats["requests"] += 1
                    continue
                outcome = events.to_outcome_record(event)
                if outcome is not None:
                    rid = by_match.get(outcome["match_key"])
                    if rid:
                        self.ledger.append({
                            "req_id": rid,
...
**类型一致性核对**：`match_key` 在 `events.py` 定义、`collector.py` 与 `recon` 夹具中使用，签名一致；
